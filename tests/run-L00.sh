#!/bin/sh
# Run all L00 tests: emit unit checks, no-C policy, per-file driver, repeatability.
# On Apple Silicon this assembles+links only .s files and compares stdout
# to .expected. On other hosts it still generates program.s and checks the
# L00 asm contract.
set -e
