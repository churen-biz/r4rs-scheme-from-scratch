...
forms = 用 L43 的读法读该文件全部 datum
        （编译器宿主侧：打开文件，循环 rt_read 或等价的 Scheme reader；
         若编译器仍跑在宿主上，允许用宿主 read 读文件，但注释、符号规则须与 L43 一致。
         推荐：编译器用同一份 reader 算法的 Scheme 移植，或 FFI 调 rt_read。）
...
2. C 符号表（字符串 → ptr）。此时 `x24` 仍须保存一个能被 GC 看见的根（例如指向「所有全局值的 vector」）。不要只把值藏在 GC 扫不到的 C `malloc` 里。
...
- `load` 不在运行时打开文件（字符串字面已在编译期展开）。用 `strace`/`dtruss` 非必须；源码路径上 `rt_read` 只用于用户 `(read)` 与 **编译器宿主** 打开 load 的文件。
...
- **忘记保存 `x24`**：从 C 来的 `scheme_entry` 必须 callee-saved；否则 `main` 返回后栈烂（本进程可能仍能印结果）。养成与 `x19` 同样保存。
