...
增加字符立即数。源程序里的 `#\A`、`#\nul`、`#\space`、`#\newline` 等编码进 64 位字：低 8 位为 `CHAR_TAG=0x0F`，字符码在 bit[15:8]。`rt_print` 按 R4RS `write` 风格打印 `#\a`、`#\space`、`#\newline`，其余用 `#\xHH` 或字面 `#\X`。自托管前用手写 `.s` 发出这些立即数，不要用宿主编译器 `read`。
...
5. `#\space` → `#\space`
6. `#\newline` → `#\newline`
7. `#\nul`（code 0）：→ `#\x00`（按上面打印合同）。测例用能表示 code 0 的写法。
8. 码点 1（SOH）：→ `#\x01`
9. 码点 127（DEL）：→ `#\x7F` 或 `#\x7f`（锁定小写十六进制、两位、`#\x` 前缀：`#\x7F`）。请实现为 **两位大写** hex，与骨架 `#\x%02X` 一致。
10. `#\(` → `#\(`
11. `#f` → `#f`；`()` → `()`（回归）
12. 输入 65：→ `65`（不是 `#\A`）
13. 字符码点 > 255：编译期错误（自托管后）或本层测例不包含。
