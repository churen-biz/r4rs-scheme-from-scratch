...
| 派生语法 | 是 | `cond`（含 `=>`，L40）`case` `and` `or` `quasiquote`；L54 的 named let、`do`、`delay` |
...
2. 从 `GAPS.md` 挑 L54 开放项（I/O、`gcd`、局部 syntax…）当课后练习。
...
无 L56。教程结束。下一步：按 `backend/README.md` 的 x86_64-linux 清单换芯片，或补 L54 仍开放项（I/O、`gcd`、局部 syntax、有理数等）。IR、标签与 `tests/L00`–`tests/L55` 保持不动。
