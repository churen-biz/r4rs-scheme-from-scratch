...
6. **更大的 n（防「栈碰巧很大」假绿）**  
   `(f 100000 0)` → `5000050000`（若 `_rt_print` 的 fixnum 范围装得下；`100000*100001/2 = 5000050000`，小于 `2^61`）。不得 SIGSEGV。
...
- **忘记自由变量**：循环里 `SELF` 被某个 prim 调用的 Darwin 整数约定 毁掉。对 runtime 辅助的 `bl` 前要按约定保存 `x19–x21`；自身纯 Scheme 循环不应碰 `x21`。
