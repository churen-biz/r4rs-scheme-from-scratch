...
``x30``. Heap base arrives in ``x0`` and size in ``x1`` per the Darwin/ARM64
integer calling convention (same register roles as AAPCS64), but this layer
does not copy them into ``HP`` (``x19``). That wait-until-L12 choice is
allowed by layers/L00-pipeline.md; L12 will add callee-saved ``x19``/``x20``
without changing the two-argument entry.
"""
